...
- `v0.3.8` `bugfix`（2026-04-06）
  - 修复 `getObsId` 兼容增强引入的回归失败：
    - `getObsId` 改为“保守优先”双 payload 尝试：先用历史稳定格式 `files:[{path}]`，失败再尝试扩展对象格式。
    - path 变体回退收敛为仅处理前导 `/` 差异，不再尝试去 bucket 前缀，避免命中错误对象导致原本可下载 case 失败。
    - 兼容增强保留，但优先级调整为“先不破坏已成功链路”。
