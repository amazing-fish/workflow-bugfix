...
- `v0.3.13` `bugfix`（2026-04-07）
  - 解码阶段日志降噪：
    - 默认关闭 camera bag 级逐条 `[OK] ... center_delta ... decoded=...` 输出，避免同任务多路相机重复刷屏；
    - 新增 task 级单行解码汇总日志：`解码完成: bags=x/y, frames=a/b, status=...`；
    - 如需排查 bag 级细节，可通过 `config.debug=true` 或 `decoder.verbose=true` 临时开启详细日志。

- `v0.3.12` `bugfix`（2026-04-07）
  - 无时间戳任务场景语义修复：
    - 当 row 未解析到碰撞时间戳（`tasks=0`）时，row 状态改为 `timestamp_missing`，不再误导性输出 AI 计数结果；
    - row 汇总日志改为明确告警：`无可执行时间戳任务，已跳过 AI/解码汇总`，并附带具体原因（如 `未检测到时间戳`）。

- `v0.3.11` `bugfix`（2026-04-07）
  - 下载阶段日志降噪：
    - 将 topic 级逐条“下载完成”日志降级为 debug 输出，默认不再逐条打印；
    - 将 row 级“就绪”日志降级为 debug 输出，常规流程仅保留 workflow 侧单行提交日志。

- `v0.3.10` `bugfix`（2026-04-07）
  - 任务顺序稳定性修复（自然数排序）：
    - 修复 row / task 在字符串排序下出现的数字错位（如 `row_10` 早于 `row_2`、`t10` 早于 `t2`）；
    - workflow 汇总、row task 摘要与 row analysis 统一改为自然顺序；
    - 监控 GUI 的 row 遍历与任务列表展示统一改为自然顺序，保证“执行观察顺序”与“任务列表顺序”一致。
